...
Fresh $WorkRoot/e2e/f1
MkVid "$WorkRoot/e2e/f1/een x264.mkv" 8
'a' > "$WorkRoot/e2e/f1/een x264.srt"
'b' > "$WorkRoot/e2e/f1/een x264.en.forced.srt"
'c' > "$WorkRoot/e2e/f1/een x264.idx"
'd' > "$WorkRoot/e2e/f1/een x264.sub"
'x' > "$WorkRoot/e2e/f1/een x264b.srt"
Reset-Run (Std-Settings -DeleteOrig $true -Subs $true)
$j = New-Job -FullPath "$WorkRoot/e2e/f1/een x264.mkv" -Dur 8.0
Enqueue-Jobs @($j)
$w = Start-W $ConvertWorker 'conv'; while (-not $w.Handle.IsCompleted) { Start-Sleep -Milliseconds 200 }; Stop-W $w | Out-Null
Drain-Log | Out-Null
$f1 = @(Get-ChildItem $WorkRoot/e2e/f1 -File | ForEach-Object Name | Sort-Object)
...
Fresh $WorkRoot/e2e/f2
MkVid "$WorkRoot/e2e/f2/twee x264.mkv" 6
'a' > "$WorkRoot/e2e/f2/twee x264.nl.srt"
Reset-Run (Std-Settings -DeleteOrig $false -Subs $true)
$j2 = New-Job -FullPath "$WorkRoot/e2e/f2/twee x264.mkv" -Dur 6.0
Enqueue-Jobs @($j2)
$w = Start-W $ConvertWorker 'conv'; while (-not $w.Handle.IsCompleted) { Start-Sleep -Milliseconds 200 }; Stop-W $w | Out-Null
Drain-Log | Out-Null
$f2 = @(Get-ChildItem $WorkRoot/e2e/f2 -File | ForEach-Object Name)
...
Fresh $WorkRoot/e2e/f3
'kapot' > $WorkRoot/e2e/f3/k1.mkv
MkVid "$WorkRoot/e2e/f3/goed_x264.mkv" 4
'kapot' > $WorkRoot/e2e/f3/k2.mkv
'kapot' > $WorkRoot/e2e/f3/k3.mkv
'kapot' > $WorkRoot/e2e/f3/k4.mkv
MkVid "$WorkRoot/e2e/f3/laatste_x264.mkv" 4
Reset-Run (Std-Settings -DeleteOrig $true -Subs $false)
$lst = @()
foreach ($n in 'k1.mkv','goed_x264.mkv','k2.mkv','k3.mkv','k4.mkv','laatste_x264.mkv') {
  $lst += (New-Job -FullPath "$WorkRoot/e2e/f3/$n" -Dur 4.0)
}
Enqueue-Jobs $lst
$w = Start-W $ConvertWorker 'conv'; while (-not $w.Handle.IsCompleted) { Start-Sleep -Milliseconds 200 }; Stop-W $w | Out-Null
Drain-Log | Out-Null
...
Fresh $WorkRoot/e2e/f4
foreach ($i in 1..5) { MkVid "$WorkRoot/e2e/f4/v$i`_x264.mkv" 5 }
Reset-Run (Std-Settings -DeleteOrig $true -Subs $false)
$lst4 = @(); foreach ($i in 1..5) { $lst4 += (New-Job -FullPath "$WorkRoot/e2e/f4/v$i`_x264.mkv" -Dur 5.0) }
Enqueue-Jobs $lst4
...
$out4  = @(Get-ChildItem $WorkRoot/e2e/f4 -File -Filter '*.x265.mkv')
...
Check 'werkmap opgeruimd'           (@(Get-ChildItem $WorkRoot/x265work -Filter 'x265_*' -ErrorAction SilentlyContinue).Count -eq 0)
''
