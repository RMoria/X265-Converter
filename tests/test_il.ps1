...
function MkSrc { param($P,[int]$Sec=60)
  Set-Content -Path /tmp/t2.srt -Value "1`n00:00:02,000 --> 00:00:05,000`neen`n`n2`n00:00:45,000 --> 00:00:50,000`ntwee`n"
  & $FFMPEG -hide_banner -loglevel error -y `
    -f lavfi -i "testsrc=size=320x240:rate=25:duration=$Sec" `
    -f lavfi -i "sine=frequency=440:duration=$Sec" -i /tmp/t2.srt `
...
& $FFMPEG -hide_banner -loglevel error -y -f lavfi -i "testsrc=size=320x240:rate=25:duration=120" -f lavfi -i "sine=duration=20" -map 0:v -map 1:a -c:v libx264 -preset ultrafast -crf 34 -c:a aac /tmp/ilt/kort.mkv 2>$null | Out-Null
$m = & (Get-Process -Id $PID).Path -NoProfile -Command @"
