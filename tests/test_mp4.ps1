...
& $FF -v error -y -f lavfi -i "testsrc=size=320x240:rate=25:duration=10" -f lavfi -i "sine=duration=10" `
  -map 0:v -map 1:a -c:v libx264 -preset ultrafast -crf 36 -c:a aac -timecode 00:00:00:00 '/tmp/m4/a/tc x264.mp4' 2>$null | Out-Null
Check 'bron heeft een datastroom'    ((Streams '/tmp/m4/a/tc x264.mp4') -match 'data')  ("-> " + (Streams '/tmp/m4/a/tc x264.mp4'))
$log = Run /tmp/m4/a 'tc x264.mp4' 10
...
Fresh /tmp/m4/b
Set-Content -Path /tmp/m4/s.srt -Value "1`n00:00:01,000 --> 00:00:03,000`nhallo`n"
& $FF -v error -y -f lavfi -i "testsrc=size=320x240:rate=25:duration=10" -f lavfi -i "sine=duration=10" -i /tmp/m4/s.srt `
  -map 0:v -map 1:a -map 2:s -c:v libx264 -preset ultrafast -crf 36 -c:a aac -c:s mov_text -timecode 00:00:00:00 '/tmp/m4/b/mt x264.mp4' 2>$null | Out-Null
Check 'bron: mov_text + data'        ((Streams '/tmp/m4/b/mt x264.mp4') -match 'subtitle' -and (Streams '/tmp/m4/b/mt x264.mp4') -match 'data')
$log = Run /tmp/m4/b 'mt x264.mp4' 10
Check 'conversie geslaagd'           ($sync.Success -eq 1)                              "(succ=$($sync.Success) fail=$($sync.Failed))"
Check 'geen herpoging nodig'         (-not (($log -join ' ') -match 'Eerste poging mislukt'))
...
Fresh /tmp/m4/c
& $FF -v error -y -f lavfi -i "color=c=red:s=64x64:d=1" -frames:v 1 /tmp/m4/cover.png 2>$null | Out-Null
& $FF -v error -y -i '/tmp/m4/a/tc x264.mp4' -i /tmp/m4/cover.png -map 0:v -map 0:a -map 1 `
  -c copy -disposition:v:1 attached_pic '/tmp/m4/c/cov x264.mp4' 2>$null | Out-Null
Check 'bron heeft 2 videosporen'     ((@((Streams '/tmp/m4/c/cov x264.mp4') -split ',') | Where-Object { $_ -eq 'video' }).Count -eq 2)
